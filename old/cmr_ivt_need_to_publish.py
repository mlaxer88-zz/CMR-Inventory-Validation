...
# Mike Laxer
#
# Script for finding datasets in V0 that need to be published
# Works by comparing list of datasets that of "Published|Published Deferred QC|Published No QC" to
# list of datasets in CMR NSIDCV0 provider that are not behind ACL's,
# and therefore can be seen by unregistered users.
#
#
#
#
...
with open('./json/nsidcv0_auth_out.json','r') as v0_file:
	v0_json = json.load(v0_file)

with open('./json/nsidcecs_auth_out.json','r') as ecs_file:
...
matches = [c for c in edb_pub if c not in cmr_list]

print "\nNumber of V0/ECS datasets in CMR: %s" % len(cmr_list)
#print "Number of EDB datasests from AGDC and/or are Brokered: %s" % len(ab_results)
...
print "Number of datasets published in EDB but not published to CMR: %s\n" % len(matches)

print "Investigate the following collections and determine if they should be published to CMR:\n"
